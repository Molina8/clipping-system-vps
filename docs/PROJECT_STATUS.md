...
- **2026-09-07 07:38 UTC** — **Whop Content Rewards public discover ✅**. Sin OAuth, sin Account API key, sin headless browser. Nuevo módulo `app/whop_integration/discover_fetcher.py` que hace GET a `https://b4e0vdqv6zgqeqj4pfgm.apps.whop.com/discover` (URL base descubierta desde `consumerViewUrlTemplate` de la app `app_QRxsQodZgK1r4D`) y parsea el RSC payload de Next.js para extraer el array `campaigns`. **50 campañas activas extraídas** con metadata completa (id, title, brand, description, type, cpm_label, budget_total/spent/available, creator_count, platforms, category, thumbnail, avatar, funded_ago, is_verified, requires_application, progress_percentage, organization_experience_id). Tests: 17 nuevos, **167/167 pytest verde** (incluye 1 test live que verifica que la API responde). `requests==2.34.2` añadido a `requirements.txt` (el venv no lo tenía). Próximo paso: cablear este fetcher al Step 1+2 del CRON scanner.

- **2026-09-07 09:21 UTC** — **Whop Content Rewards campaign detail fetcher ✅**. Nuevo módulo `app/whop_integration/campaign_detail_fetcher.py` (530 líneas, 6 dataclasses: `PayoutRule`, `ReferenceMaterial`, `TopEarner`, `DailyMetric`, `CampaignDetail`, fetcher). Hace GET a `https://b4e0vdqv6zgqeqj4pfgm.apps.whop.com/campaigns/<id>` (sin auth) y parsea el RSC payload con brace matching **string-aware** para extraer el objeto de campaign completo. Devuelve `CampaignDetail` con: identity (id, name, brand, verified, status, private), payout rules por plataforma (rate/min/max en cents + USD helpers), plataformas aceptadas, reference materials (URLs a brand assets), budget (total/spent/progress/paid_out/available con helpers USD), gating flags (requires_application, retainer_spots_total), creator metrics (creator_count, total_views, approved_submissions, top_earners, chart_points por día), y assets (banner, logo). Métodos auxiliares: `is_active`, `has_drive_assets`, `payout_for_platform(platform)`. Tests: 20 nuevos, **187/187 pytest verde** (incluye test live contra el endpoint real). Side fixes: bug en backward walk donde `in_string` se iniciaba en False cuando debería ser True (la marker `"status":"active"` empieza con `"`, así que estamos dentro del string en idx); fix: empezar el walk desde `idx-1`.

- **2026-09-07 10:17 UTC** — **Whop Drive fetcher ✅**. Nuevo módulo `app/whop_integration/drive_fetcher.py` (351 líneas). Parsea folders públicos de Google Drive (`?usp=sharing`) y descarga archivos sin auth. Métodos: `list_folder(url_or_id)` → `list[DriveFile]`, `download_file(file_url_or_id, dest_dir)` → `Path`, `iter_files(url_or_id)`. Dataclass `DriveFile` con `id`, `name`, `is_folder`, `mime_type`, `size_bytes`, `modified_at`, `folder_url`, `download_url`, `view_url`. Helpers: `is_drive_url(url)`, `extract_drive_id(url)`. Maneja confirm page de Drive para archivos >100MB (`download_warning_` cookie + form action). Tests: 32 nuevos (DriveFile dataclass + URL helpers + URL normalization + file ID extraction + HTML parsing del folder de Yomi Denzel + Content-Disposition parsing + defaults + failures + live download). **218/218 pytest verde** (187 previos + 32 nuevos - 1 skip live). Side fixes: test ids cambiados de 12-21 chars a 33 chars (longitud real de Drive ids); `_parse_filename` reescrito sin regex sobre Content-Disposition (string slicing puro para evitar escape hell con raw strings en heredocs).
